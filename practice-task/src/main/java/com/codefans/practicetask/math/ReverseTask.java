...
public class ReverseTask extends AbstractUnitPriceReverse implements Callable<Result> {
    ...
    public Result calculate(BigDecimal totalPrice, List<BigDecimal> weightList, List<BigDecimal> defaultUnitPriceList) {
        ...
        this.print(beginIndexList);
        this.print(endIndexList);
        this.print(lengthList);
        this.print(currentIndexList);
//        this.print(unitPriceList);
        ...
        if(upPriceStep != null) {
            if(CollectionUtils.isNotEmpty(ifSortList)) {
                ...
                if(ifSort) {
                    ...
                    //如果不参与排序, 则下标就是单价本身
                    return generateEndIndex(unitPrice, Zero, priceIndexRadio);
                }
            } else {
                return generateEndIndex(unitPrice, upPriceStep, priceIndexRadio);
            }
