...
public abstract class AbstractUnitPriceReverse {
    ...
    public BigDecimal[] getNewUnitPrices(BigDecimal unitPriceStep, BigDecimal[] defaultUnitPrices) {
        ...
        for(int i = 0; i < defaultUnitPrices.length; i ++) {
            if(unitPriceStep != null) {
                addPrice = this.add(defaultUnitPrices[i], unitPriceStep);
                //如果浮动一定步长后价格小于等于0, 则价格不浮动, 还是取原来的值
    ...
    public int generateBeginIndex(BigDecimal defaultPrice, BigDecimal downPriceStep, BigDecimal priceIndexRadio) {
        int beginIndex = 0;
        BigDecimal subPrice = subtract(defaultPrice, downPriceStep);
        //如果向下浮动给你步长后价格小于等于0,则价格为0.01
        if(subPrice.compareTo(BigDecimal.ZERO) <= 0) {
            //按百分比减能尽可能的接近原值, 而不会差太多
            subPrice = subtract(defaultPrice, multiply(defaultPrice, devide(defaultPrice,downPriceStep)));
            if(subPrice.compareTo(BigDecimal.ZERO) <= 0) {
                subPrice = new BigDecimal("0.01");
            }
        }
        beginIndex = this.multiply(subPrice,priceIndexRadio).intValue();
        return beginIndex;
    }
