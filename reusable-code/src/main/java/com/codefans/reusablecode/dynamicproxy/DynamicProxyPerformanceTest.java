 ...
 *
 * 软件环境：
 *    jdk1.8.0_162、javassist-3.25.0-GA、asm-7.1、cglib-3.2.12
 * 硬件环境：
 ...
 *
 * 测试结果：
 * Create JDK Proxy: 6 ms
 * Create CGLIB Proxy: 118 ms
 * Create JAVASSIST Proxy: 97 ms
 * Create JAVASSIST Bytecode Proxy: 75 ms
 * Create ASM Proxy: 1 ms
 * ================
 * Run JDK Proxy: 55 ms, 25,637,552 t/s
 * Run CGLIB Proxy: 54 ms, 26,112,322 t/s
 * Run JAVASSIST Proxy: 179 ms, 7,877,460 t/s
 * Run JAVASSIST Bytecode Proxy: 50 ms, 28,201,308 t/s
 * Run ASM Bytecode Proxy: 45 ms, 31,334,786 t/s
 * ----------------
 * Run JDK Proxy: 101 ms, 13,961,043 t/s
 * Run CGLIB Proxy: 64 ms, 22,032,272 t/s
 * Run JAVASSIST Proxy: 224 ms, 6,294,934 t/s
 * Run JAVASSIST Bytecode Proxy: 49 ms, 28,776,845 t/s
 * Run ASM Bytecode Proxy: 43 ms, 32,792,218 t/s
 * ----------------
 * Run JDK Proxy: 66 ms, 21,364,627 t/s
 * Run CGLIB Proxy: 57 ms, 24,737,989 t/s
 * Run JAVASSIST Proxy: 220 ms, 6,409,388 t/s
 * Run JAVASSIST Bytecode Proxy: 41 ms, 34,391,839 t/s
 * Run ASM Bytecode Proxy: 39 ms, 36,155,523 t/s
 * ----------------
 *
 * 测试结论：
 *    1.javassist和asm的字节码方式最快，二者不相上下。
 *    2.其次是cglib（底层也是用的asm）
 *    3.再次是jdk，jdk也是通过字节码的方式实现动态代理的
 *    4.最慢的是javassist动态代理接口
 *
 * (三) 差异原因：
 * 各方案生成的字节码不一样，
